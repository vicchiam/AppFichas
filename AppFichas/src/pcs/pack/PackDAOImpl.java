...
public class PackDAOImpl implements PackDAO{
	...
	private String SELECT_SQL="SELECT id, description, mesure, weight, apt, state FROM pack";
	private String SELECT_ID_SQL="SELECT id, description, mesure, weight, apt, state FROM pack WHERE id=?";
	private String INSERT_SQL="INSERT INTO pack (description, mesure, weight, apt) VALUES(?,?,?,?)";
	private String UPDATE_SQL="UPDATE pack SET description=?, mesure=?, weight=?, apt=? WHERE id=?";
	...
	public Pack getPack(int id) throws SQLException {
		...
		params.add(id);
		return this.genericDAO.get(SELECT_ID_SQL, params, new Pack());
	}
	...
	private Collection<Pack> filter(String description, int apt, int state) throws SQLException{
		...
		return this.genericDAO.list(sql, params, new Pack());					
	}
